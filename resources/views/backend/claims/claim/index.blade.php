...
<h5 class="font-weight-normal">Claim </h5>
</div>
...
<a href="{{route('claims.claim.edit')}}" class="btn btn-success btn-icon m-2" ><i class="fas fa-plus-square"></i>&nbsp; Update Claim Page</a>
...
<!-- Table body -->
<tbody>
@foreach ($claim as $index => $cl)
    <tr>
        <td>{{ $index + 1 }}</td>
        <td>
            <img src="{{ asset('uploads/claim/'.$cl->banner) }}" alt="" class="img-fluid" width="100">
        </td>
        <td>{{ $cl->banner_title_en }}</td>
        <td>{{ $cl->banner_title_bn }}</td>
        <td>{{ $cl->name_en }}</td>
        <td>{{ $cl->name_bn }}</td>
